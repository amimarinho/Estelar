...
import { EnergyOrb } from '@/components/index';
...
export default function GuidedBreathingScreen() {
  ...
  useEffect(() => {
    ...
    if (currentPhase === 'inspire') {
      scaleVal.value = withTiming(1.4, { duration: 4000, easing: Easing.inOut(Easing.ease) });
    } else if (currentPhase === 'segure') {
      scaleVal.value = 1.4;
    } else if (currentPhase === 'expire') {
      scaleVal.value = withTiming(1.0, { duration: 4000, easing: Easing.inOut(Easing.ease) });
    } else if (currentPhase === 'descanse') {
      scaleVal.value = 1.0;
    }
  ...
  return (
    <View className="flex-1 bg-surface relative">
      ...
      <SafeAreaView className="flex-1 z-10" edges={['top', 'bottom']}>
        ...
        >
          ...
          <View className="mb-6 items-center">
            ...
            >
              Inspire, segure e solte no ritmo indicado. Concentre-se apenas no próximo ciclo.
            </Animated.Text>

            <View className="w-64 h-64 items-center justify-center relative mb-8">
              <Animated.View style={[animatedOrbStyle]} className="absolute inset-0 items-center justify-center">
                <EnergyOrb
                  width={240}
                  height={240}
                  speed={orbSpeed}
            ...
            <View className="flex-row justify-center items-center w-full mt-2 mb-4 px-2">
              {(['inspire', 'segure', 'expire', 'descanse'] as Phase[]).map((phase, idx) => {
                ...
                return (
                  <View key={phase} className="flex-row items-center">
                    <View className="items-center">
                      ...
                      >
                        {isActive && <View className="w-1.5 h-1.5 rounded-full bg-[#17142a]" />}
                      </View>
                      <Text 
                        className={`font-sans text-[10px] mt-2 ${
                          isActive ? 'text-primary font-bold' : 'text-text-muted/60'
                        }`}
                      >
                        {getPhaseTitle(phase)}
                      </Text>
                    ...
                    {idx < 3 && (
                      <View 
                        className={`h-[1px] w-8 mx-1 mb-5 border-t border-dashed ${
                          isPlaying && idx < ['inspire', 'segure', 'expire', 'descanse'].indexOf(currentPhase)
                            ? 'border-primary' 
                            : 'border-stroke-soft'
                        }`}
                      />
              ...
              {([1, 3, 5] as const).map((mins) => {
                ...
                return (
                  <Pressable
                    key={mins}
                    disabled={isPlaying}
                    onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); setSelectedDuration(mins); }}
          ...
          <View className="space-y-4 mb-6">
            ...
            <Pressable
              onPress={() => {
                Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
                Alert.alert("Sons Calmantes", "Iniciando tocador de frequências calmantes e ruído rosa...");
              }}
            ...
            <Text className="font-sans text-[11px] text-text-muted/60 text-center leading-relaxed px-6 mt-4">
              Este exercício pode ser usado antes de dormir, após conflitos ou em momentos de ansiedade.
            </Text>
          </View>
        </ScrollView>
      </SafeAreaView>
      ...
      {showSuccessModal && (
        <View style={StyleSheet.absoluteFillObject} className="z-50 items-center justify-center bg-surface/85 px-6">
          <Animated.View
            entering={FadeIn.duration(400)}
          ...
          >
            <View className="w-16 h-16 rounded-full bg-feedback-success/10 items-center justify-center mb-6">
              <Ionicons name="checkmark-circle-outline" size={36} color="#8fe3b0" />
            </View>
            ...
            <Text className="font-sans text-xs text-text-muted text-center leading-relaxed px-2 mb-8">
              Você completou a pausa de respiração guiada. Como está se sentindo agora?
            </Text>
