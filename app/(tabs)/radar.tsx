...
export default function RadarScreen() {
  ...
  const getMoodIndicator = () => {
    if (!lastCheckin) return { text: 'Nenhum', color: '#b8bde0', icon: 'help-circle-outline' as const };
    if (lastCheckin.mood === 'calmo') return { text: 'Calmo', color: '#8fe3b0', icon: 'leaf-outline' as const };
    if (lastCheckin.mood === 'bem') return { text: 'Bem', color: '#8fe3b0', icon: 'happy-outline' as const };
    if (lastCheckin.mood === 'instavel') return { text: 'Instável', color: '#ffd66b', icon: 'pulse-outline' as const };
    if (lastCheckin.mood === 'ansioso') return { text: 'Ansioso', color: '#ff8a70', icon: 'sync-outline' as const };
    return { text: 'Sobrecarregado', color: '#ff8a8a', icon: 'layers-outline' as const };
  };

  const getStressIndicator = () => {
    if (!lastCheckin) return { text: 'Nenhum', color: '#b8bde0' };
    if (lastCheckin.stressLevel === 0) return { text: 'Baixo', color: '#8fe3b0' };
    if (lastCheckin.stressLevel === 1) return { text: 'Moderado', color: '#ffd66b' };
    return { text: 'Alto', color: '#ff8a8a' };
  };

  const getEnergyIndicator = () => {
    if (!lastCheckin) return { text: 'Nenhuma', color: '#b8bde0', icon: 'battery-half-outline' as const };
    if (lastCheckin.energyLevel === 0) return { text: 'Baixa', color: '#ff8a8a', icon: 'battery-dead-outline' as const };
    if (lastCheckin.energyLevel === 1) return { text: 'Média', color: '#ffd66b', icon: 'battery-half-outline' as const };
    return { text: 'Alta', color: '#8fe3b0', icon: 'battery-full-outline' as const };
  };

  const getSleepIndicator = () => {
    if (!lastCheckin) return { text: 'Nenhum', color: '#b8bde0' };
    if (lastCheckin.sleepQuality === 0) return { text: 'Baixo', color: '#ff8a8a' };
    if (lastCheckin.sleepQuality === 1) return { text: 'Médio', color: '#ffd66b' };
  ...
  const getRiskText = () => {
    if (percentage < 35) return { primary: 'Estabilidade emocional', highlight: 'ideal' };
    if (percentage < 65) return { primary: 'Sobrecarga emocional', highlight: 'leve' };
    if (percentage < 85) return { primary: 'Sobrecarga emocional', highlight: 'moderada' };
        ...
        >
            ...
            <View className="flex-row items-center">
              <View className="relative" style={{ width: size, height: size }}>
                ...
                <View className="absolute inset-0 items-center justify-center pt-1">
                  ...
                  <Text className="font-mono text-xs font-bold text-text-high mt-0.5">{percentage}%</Text>
                </View>
              </View>
              ...
              <View className="flex-1 ml-5">
                ...
                <Text 
                  style={{
                    color: 
                      riskText.highlight === 'ideal' ? '#8fe3b0' :
                      riskText.highlight === 'leve' ? '#ffd66b' :
                      riskText.highlight === 'moderada' ? '#ff8a70' : '#ff8a8a'
                  }}
            ...
            <View className="flex-row justify-between items-center">
              <View className="items-center flex-1">
                <View className="w-12 h-12 rounded-full bg-surface items-center justify-center border border-stroke-soft">
                  <Ionicons name={moodInd.icon} size={20} color={moodInd.color} />
                </View>
                <Text className="font-mono text-[8px] text-text-muted uppercase tracking-[1px] mt-2">
                  HUMOR
                </Text>
                <Text style={{ color: moodInd.color }} className="font-sans text-[11px] font-semibold mt-0.5">
                  {moodInd.text}
                </Text>
              </View>
              ...
              <View className="items-center flex-1">
                <View className="w-12 h-12 rounded-full bg-surface items-center justify-center border border-stroke-soft">
                  <Ionicons name="alert-circle-outline" size={20} color={stressInd.color} />
                </View>
                <Text className="font-mono text-[8px] text-text-muted uppercase tracking-[1px] mt-2">
                  ESTRESSE
                </Text>
                <Text style={{ color: stressInd.color }} className="font-sans text-[11px] font-semibold mt-0.5">
                  {stressInd.text}
                </Text>
              </View>
              ...
              <View className="items-center flex-1">
                <View className="w-12 h-12 rounded-full bg-surface items-center justify-center border border-stroke-soft">
                  <Ionicons name={energyInd.icon} size={20} color={energyInd.color} />
                </View>
                <Text className="font-mono text-[8px] text-text-muted uppercase tracking-[1px] mt-2">
                  ENERGIA
                </Text>
                <Text style={{ color: energyInd.color }} className="font-sans text-[11px] font-semibold mt-0.5">
                  {energyInd.text}
                </Text>
              </View>
              ...
              <View className="items-center flex-1">
                <View className="w-12 h-12 rounded-full bg-surface items-center justify-center border border-stroke-soft">
                  <Ionicons name="moon-outline" size={20} color={sleepInd.color} />
                </View>
                <Text className="font-mono text-[8px] text-text-muted uppercase tracking-[1px] mt-2">
                  SONO
                </Text>
                <Text style={{ color: sleepInd.color }} className="font-sans text-[11px] font-semibold mt-0.5">
                  {sleepInd.text}
                </Text>
              </View>
            </View>
            ...
            <View className="flex-row items-start">
              ...
              <View className="flex-1">
                ...
                <Text className="font-sans text-sm text-text-high leading-relaxed mt-1.5">
                  Recomendamos uma pausa guiada ou o envio de um relato para suporte psicológico.
                </Text>
              </View>
            </View>
          ...
          <View className="bg-surface-card rounded-[28px] p-6 border border-primary/10 mb-8">
            ...
            <Text className="font-sans text-[10px] text-text-muted/60 text-center leading-relaxed mt-4 px-2">
              O Estelar identifica padrões de risco sem substituir o acompanhamento humano.
            </Text>
          </View>
        </ScrollView>
      </SafeAreaView>
